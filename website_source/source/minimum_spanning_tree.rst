...
.. code-block:: python
    ...
    mst = ly.GetMST(x=x, y=y) # Assuming our input data set is 2D.
...
The parallel class was designed to be used on large N-body simulations. For this
reason its usability is limited. What this class will do is take a large 3D data
set and split the data into a specified number of sections. The MST algorithm is
then run on each section and the data is saved.

Let's imagine we have a large N-body simulation data set and we are going to use
the parallel class to split the data set into 3 groups along 1 axis, meaning 27
in total.
...
.. code-block:: python
    ...
    # Coordinates of the simulations particles, galaxies or haloes.
    x, y, z =
    # The size of the 3D cube in which the simulation inhabits.
    box_size =
    # The number of sections to split the data set into in each axis.
    # Total sections is divisions**3.
    divisions =
    # The number of processors you wish to use (default=1).
    parallel =

    # OPTIONAL:

    # Number of divisions carried out to find branches for each individually
    # constructed MST. (default: 4)
    branch_sub_divisions =
    # Number of k-nearest neighbours. (default: 20)
    k_neighbours =
    # Scale cut length. (default: 0.)
    scale_cut_length =

    xmst.setup(x, y, z box_size, divisions, parallel=parallel,
               branch_sub_divisions=branch_sub_divisions,
               k_neighbours=k_neighbours, scale_cut_length=scale_cut_length)

    # Runs the parallel code.
    xmst.run()

    # Combines all the statistics from each section.
    d, l, b, s = xmst.get_data()
